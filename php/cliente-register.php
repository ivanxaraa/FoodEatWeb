...
if ($register) {
    ...
    $sql = "SELECT id FROM cliente WHERE username = :USERNAME LIMIT 1";
    $stmt = $pdo->prepare($sql);
    ...
    if (!$errors) {        
        $sql = "INSERT INTO cliente(username,email,password,status) VALUES(:USERNAME,:EMAIL,:PASSWORD,1)";
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(":USERNAME", $username, PDO::PARAM_STR);
        $stmt->bindValue(":EMAIL", $email, PDO::PARAM_STR);
        ...
        <div class="login-container">
            ...
            <form action="?" method="POST">
                <div class="login-content">
                    <span>Registar <span>cliente</span></span>
                    <div class="login-input">
                        <input type="text" placeholder="Username" name="username" id="username" required>
                        <input type="email" placeholder="Email" name="email" id="email" required>
                        <input type="password" placeholder="Password" name="password" id="input-password" onkeyup="InputError(this.value)" required>
                    </div>
